                ...
                <div class="d-flex justify-content-between">
                            ...
                            <ol class="breadcrumb breadcrumb-style1 mb-0">
                                ...
                                <li class="breadcrumb-item active fs-4">
                                    <span class="fw-bold text-primary d-flex align-items-center">
                                        ...
                                        <span class="ms-1">مؤسسة المدرب</span>
                                    </span>
                                </li>
                            </ol>
                        </nav>
                    ...
                    <div>
                        <div class="d-flex gap-2">
                            ...
                            @if(auth()->user()->can('traininginstitution-export-excel') || auth()->user()->can('traininginstitution-export-pdf'))
                                <div class="btn-group" role="group">
                                    ...
                                    <ul class="dropdown-menu">
                                        @can('traininginstitution-export-excel')
                                            <li>
                                        ...
                                        @endcan
                                        @can('traininginstitution-export-pdf')
                                            <li>
                                                <a class="dropdown-item" href="{{ route('TrainingInstitutions.export.pdf.tcpdf') }}">
                                                    <i class="mdi mdi-file-pdf-box me-2 text-danger"></i>
                                                    تصدير PDF (TCPDF)
                                                </a>
                                            </li>
                                            <li>
                                                <a class="dropdown-item" href="{{ route('TrainingInstitutions.print.view') }}" target="_blank">
                            ...
                            @endif
                            @can('traininginstitution-create')
                                <button wire:click='AddTrainingInstitutionModalShow' class="mb-3 add-new btn btn-primary mb-md-0"
                                    data-bs-toggle="modal" data-bs-target="#addtraininginstitutionModal">أضــافــة</button>
                            @endcan
                        </div>
                        @include('livewire.training-institutions.modals.add-training-institution')
                    </div>
                </div>
            </div>
            @can('traininginstitution-list')
                <div class="table-responsive">
                    <table class="table">
                            ...
                            <tr>
                                ...
                                <th class="text-center">
                                        <input type="text" wire:model.debounce.300ms="search.name"
                                            class="form-control text-center" placeholder="اسم المؤسسة"
                        ...
                        <tbody>
                            ...
                            @endphp
                            @foreach ($TrainingInstitutions as $TrainingInstitution)
                                <tr>
                                    <td>
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" wire:model="selectedRows"
                                                value="{{ $TrainingInstitution->id }}">
                                        </div>
                                    </td>
                                    <td>{{ $i++ }}</td>
                                    <td class="text-center">{{$TrainingInstitution->name}}</td>
                                    <td class="text-center">
                                        <div class="btn-group" role="group" aria-label="First group">
                                            @can('traininginstitution-edit')
                                                <button wire:click="GetTrainingInstitution({{$TrainingInstitution->id}})"
                                                    class="p-0 px-1 btn btn-text-primary waves-effect" data-bs-toggle="modal"
                                                    data-bs-target="#edittraininginstitutionModal">
                                                    <i class="mdi mdi-text-box-edit-outline fs-3"></i>
                                                </button>
                                            @endcan
                                            @can('traininginstitution-delete')
                                                <strong style="margin: 0 10px;">|</strong>
                                                <button wire:click="GetTrainingInstitution({{$TrainingInstitution->id}})"
                                                    class="p-0 px-1 btn btn-text-danger waves-effect"
                                                    data-bs-toggle = "modal" data-bs-target="#removetraininginstitutionModal">
                ...
                <!-- Modal -->
                @include('livewire.training-institutions.modals.edit-training-institution')
                @include('livewire.training-institutions.modals.remove-training-institution')
                <!-- Modal -->
            @endcan
        </div>
    @else
