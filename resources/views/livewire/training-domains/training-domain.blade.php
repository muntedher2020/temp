                ...
                <div class="d-flex justify-content-between">
                            ...
                            <ol class="breadcrumb breadcrumb-style1 mb-0">
                                ...
                                <li class="breadcrumb-item active fs-4">
                                    <span class="fw-bold text-primary d-flex align-items-center">
                                        ...
                                        <span class="ms-1">المجال التدريبي</span>
                                    </span>
                                </li>
                            </ol>
                        </nav>
                    ...
                    <div>
                        <div class="d-flex gap-2">
                            ...
                            @if(auth()->user()->can('trainingdomain-export-excel') || auth()->user()->can('trainingdomain-export-pdf'))
                                <div class="btn-group" role="group">
                                    ...
                                    <ul class="dropdown-menu">
                                        @can('trainingdomain-export-excel')
                                            <li>
                                        ...
                                        @endcan
                                        @can('trainingdomain-export-pdf')
                                            <li>
                                                <a class="dropdown-item" href="{{ route('TrainingDomains.export.pdf.tcpdf') }}">
                                                    <i class="mdi mdi-file-pdf-box me-2 text-danger"></i>
                                                    تصدير PDF (TCPDF)
                                                </a>
                                            </li>
                                            <li>
                                                <a class="dropdown-item" href="{{ route('TrainingDomains.print.view') }}" target="_blank">
                            ...
                            @endif
                            @can('trainingdomain-create')
                                <button wire:click='AddTrainingDomainModalShow' class="mb-3 add-new btn btn-primary mb-md-0"
                                    data-bs-toggle="modal" data-bs-target="#addtrainingdomainModal">أضــافــة</button>
                            @endcan
                        </div>
                        @include('livewire.training-domains.modals.add-training-domain')
                    </div>
                </div>
            </div>
            @can('trainingdomain-list')
                    ...
                    <table class="table">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>#</th>
                                <th class="text-center">اسم المجال</th>
                            ...
                            <tr>
                                ...
                                <th class="text-center">
                                        <input type="text" wire:model.debounce.300ms="search.name"
                                            class="form-control text-center" placeholder="اسم المجال"
                        ...
                        <tbody>
                            ...
                            @endphp
                            @foreach ($TrainingDomains as $TrainingDomain)
                                <tr>
                                    <td>
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" wire:model="selectedRows"
                                                value="{{ $TrainingDomain->id }}">
                                        </div>
                                    </td>
                                    <td>{{ $i++ }}</td>
                                    <td class="text-center">{{$TrainingDomain->name}}</td>
                                    <td class="text-center">
                                        <div class="btn-group" role="group" aria-label="First group">
                                            @can('trainingdomain-edit')
                                                <button wire:click="GetTrainingDomain({{$TrainingDomain->id}})"
                                                    class="p-0 px-1 btn btn-text-primary waves-effect" data-bs-toggle="modal"
                                                    data-bs-target="#edittrainingdomainModal">
                                                    <i class="mdi mdi-text-box-edit-outline fs-3"></i>
                                                </button>
                                            @endcan
                                            @can('trainingdomain-delete')
                                                <strong style="margin: 0 10px;">|</strong>
                                                <button wire:click="GetTrainingDomain({{$TrainingDomain->id}})"
                                                    class="p-0 px-1 btn btn-text-danger waves-effect"
                                                    data-bs-toggle = "modal" data-bs-target="#removetrainingdomainModal">
                ...
                <!-- Modal -->
                @include('livewire.training-domains.modals.edit-training-domain')
                @include('livewire.training-domains.modals.remove-training-domain')
                <!-- Modal -->
            @endcan
        </div>
    @else
