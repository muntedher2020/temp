...
class TrainerTcpdfExportController extends Controller
{
    ...
    /**
     * Export PDF for Trainers using TCPDF
     */
    public function exportPdf()
    {
        try {
            ...
            $pdf->SetTitle('تقرير المدربين');
            $pdf->SetSubject('تقرير شامل لـ المدربين');
            ...
            $pdf->setRTL(true);
            $pdf->Cell(0, 15, 'تقرير المدربين', 0, 1, 'C');
            $pdf->Ln(5);
            ...
            $pdf->Cell(24, 10, 'اسم المدرب', 1, 0, 'C', 1);
            $pdf->Cell(24, 10, 'مؤسسة المدرب', 1, 0, 'C', 1);
            $pdf->Cell(24, 10, 'التحصيل العلمي', 1, 0, 'C', 1);
            $pdf->Cell(24, 10, 'المجال التدريبي', 1, 0, 'C', 1);
            $pdf->Cell(24, 10, 'رقم الهاتف', 1, 0, 'C', 1);
            $pdf->Cell(24, 10, 'البريد الالكتروني', 1, 0, 'C', 1);
            ...
            foreach($data as $item) {
                ...
                $pdf->Cell(24, 8, $item->trainer_name ?? 'غير محدد', 1, 0, 'C', 1);
                $pdf->Cell(24, 8, $item->institution_id ?? 'غير محدد', 1, 0, 'C', 1);
                $pdf->Cell(24, 8, $item->ed_level_id ?? 'غير محدد', 1, 0, 'C', 1);
                $pdf->Cell(24, 8, $item->domain_id ?? 'غير محدد', 1, 0, 'C', 1);
                $pdf->Cell(24, 8, $item->phone ?? 'غير محدد', 1, 0, 'C', 1);
                $pdf->Cell(24, 8, $item->email ?? 'غير محدد', 1, 0, 'C', 1);
            ...
            return $pdf->Output('تقرير_المدربين_' . now()->format('Y_m_d_H_i_s') . '.pdf', 'D');
